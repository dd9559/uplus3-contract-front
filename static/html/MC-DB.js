...
let sub = {
  ...
  'info_check3': {
    stateful: function() {
      let list = Array.from(document.querySelectorAll('*[name=check3]')),
          flag = list.some(el => {
        return el.querySelector('p').getAttribute('checked')
      })
      return flag ? {"checkbox_check3": {
        state: true,
  ...
  'info_check4': {
    stateful: function() {
      let list = Array.from(document.querySelectorAll('*[name=check4]')),
          flag = list.some(el => {
        return el.querySelector('p').getAttribute('checked')
      })
      return flag ? {"checkbox_check4": {
        state: true,
        stateful: function (index) {
          let obj = null
          switch (index) {
...
contractConfig.checkboxListener(function(obj, i) {
    let name = obj.currentTarget.getAttribute('name'),
        bool = obj.currentTarget.querySelector('p').getAttribute('checked');
