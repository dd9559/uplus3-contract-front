    ...
    <div class="dialog1">
      <div v-loading="loading" class="base-dialog">
        ...
        <div class="ach-header">
          <h1 v-if="dialogType==0">业绩申诉审核</h1>
        </div>
        ...
        <div class="ach-body">
          ...
          <div class="ach-divide-list">
            <el-table :data="houseArr" style="width:100%" class="sushen">
              <!-- 角色类型 不可输入-->
          ...
          <div class="ach-divide-list">
            <el-table :data="clientArr" style="width: 100%" class="sushen">
              ...
              <el-table-column label="在职状况" width="96">
                <template slot-scope="scope">
                  <el-select v-model="scope.row.isJob" placeholder="请选择">
                    <el-option
                      v-for="item in dictionary['20']"
          ...
          <div class="ach-divide-list" style="margin-bottom:20px">
            <el-table :data="shensuArr" style="width: 100%">
              <el-table-column label="申诉人">
                <template slot-scope="scope">
                  <div
                    v-if="scope.row.appealName&&scope.row.appealName.length>0"
                  >{{scope.row.appealDepName}}-{{scope.row.appealName}}</div>
                  <div v-else>-</div>
                </template>
              </el-table-column>
              ...
              <el-table-column label="申诉内容" width="200">
                <template slot-scope="scope">
                ...
                <template slot-scope="scope">
                  <div v-if="scope.row.voucherUrl&&scope.row.voucherUrl.length>0">
                    <div v-for="(item,index) in scope.row.voucherUrl">
                      <p
                        class="link"
                        @click="previewPhoto(scope.row.voucherUrl,index)"
                      >附件{{index+1}}</p>
                    </div>
                  </div>
                  <div v-else>-</div>
              ...
              <el-table-column label="审核备注">
                <template slot-scope="scope">
                  <div v-if="scope.row.auditRemarks"  v-for="item in scope.row.approvalDetailsList" :key="item.auditInfo">
                    <el-popover
                      trigger="hover"
                      width="200"
              ...
              <el-table-column label="操作">
                <template slot-scope="scope">
                  <div v-if="scope.row.auditStatus&&scope.row.auditStatus.value==0 && scope.row.auditId == userInfo.empId" >                  
      ...
      >
        <div class="footer-btn-layout f_r">
          <el-button type="primary" round @click="passAch" class="color-green" id="savebtn">确定</el-button>
        </div>
      </div>
          ...
          <div v-loading="loading3">
            ...
            <div class="mansList">
              ...
              >
                ...
                <el-table-column label="经纪人" width="100">
                  <template slot-scope="scope">
            ...
            <div class="dialog2-btn">
              <el-button type="primary" round @click="showTips1 = false">取消</el-button>
  ...
  methods: {
    ...
    pass() {
      ...
      this.$ajax
        ...
        .catch(err => {
          console.log(err);
          this.checkPerson.code = err.data.bizCode,
          this.checkPerson.type = 3,
          this.checkPerson.flowType = err.data.flowType,
    ...
    fixedFilter(val){
      ...
      if(val.indexOf('.')!=-1){
        console.log( val.substr(0,val.indexOf('.')));
         return val.substr(0,val.indexOf('.')+1)+val.substr(val.indexOf('.')+1,2)
      }else {
        return val
      }
      return val.indexOf('.')
