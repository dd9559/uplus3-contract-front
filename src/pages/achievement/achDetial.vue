<template>
  <div>
    ...
    <div class="ach-body">
      ...
      <h1 class="f14">客源方分成
        <el-button
          type="primary"
          @click="ammanger"
        >{{$route.query.version=='0'?'AM管理关系':'师徒管理关系'}}
        </el-button>
      </h1>
      <p class="f_l delive" style="position:relative;color:red;top:-14px">房客源可分配业绩总计：{{comm?comm:0}}元</p>
      <div class="ach-divide-list">
    ...
    <el-dialog class="record-table-dialog" :closeOnClickModal="$tool.closeOnClickModal" width="770px"
               title="房源价格变更记录（近三天历史记录）" :visible.sync="recordShow">
    ...
    <el-dialog class="record-table-dialog" :closeOnClickModal="$tool.closeOnClickModal" width="770px"
               :title="$route.query.version=='0'?'AM管理关系':'师徒管理关系'" :visible.sync="AMShow">
    ...
    <el-dialog title="审核" :visible.sync="aplDialog" width="740px" @close="close"
      ...
      <div class="input-group" style="position:relative">
        ...
        <div v-if="aplurl&&aplurl.length>0">
                      ...
                      <span class="link" @click="previewPhoto(aplurl,index)"
      ...
      <div class="input-group" style="position:relative;">
        <label>备注：</label>
        <el-input type="textarea" :rows="4" resize='none' v-model="aplremark" placeholder="无备注内容"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="pass('yes')" type="primary" class="confirmBtn" v-dbClick>通过</el-button>
      </div>
    </el-dialog>
  </div>
</template>
