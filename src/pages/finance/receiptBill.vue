    ...
    <div class="view">
        <div class="view-context">
            ...
            <ul class="bill-form">
                ...
                <i class="iconfont icon-wenhao"></i>
                        ...
                        <p class="block-receipt-type worth-list"
                            v-if="firstCreate.content.contCommission">
                            <span>{{isentrust?"应收交易服务费":"应收佣金"}}（元）：{{firstCreate.content.contCommission.receivableCommission}}</span><span>已收（元）：{{firstCreate.content.contCommission.receivedCommission}}</span><span class="warning-text">未收（元）：{{firstCreate.content.contCommission.uncollected}}</span>
                        </p>
                    </div>
                </li>
                <li>
                    <div class="input-group col">
                        <div class="flex-box tool-tip">
                            ...
                            <el-tooltip content="当未找到需要的款类时，可联系管理员进行配置"
                                placement="top">
                                <p class="tip-message"><i class="iconfont icon-wenhao"></i>填写帮助</p>
                            </el-tooltip>
                        </div>
                        <moneyTypePop v-if="!isentrust"
                            ref="moneyType"
                            :data="moneyType"
                            :init="moneyTypeName"
                            @checkCell="getCell"
                        ...
                        <el-input v-else
                            ...
                            class="w200"
                            disabled
                            placeholder="交易服务费"></el-input>
                    </div>
                    <div class="input-group col active-400 address-label"
                        v-if="collect">
                        ...
                        <div class="flex-box w400">
                            ...
                            <div class="address-slelect"
                                @click="addressClickFn"><span :class="[addressLabel.name?'':'cl-gray']">{{addressLabel.name?addressLabel.name:addressLabel.val + addressValCom.name}}</span><i class="el-icon-arrow-down"></i></div>
                        </div>
                        <div class="address-label-p"
                            v-if="addressLabel.label">物业地址：{{addressLabel.label}}</div>
                    </div>
                    <div class="input-group col"
                        :class="[inputPerson?'active-360':'']">
                        ...
                        <div class="flex-box">
                            <el-select size="small"
                                ...
                                <el-option v-for="item in dropdown"
                                    :key="item.value"
                                    :label="item.label"
                                    :value="item.value">
                                </el-option>
                    ...
                    <div class="input-group col active-400">
                        ...
                        <div class="flex-box w400"
                            v-if="inObjPerson">
                            <select-tree v-if="firstCreate.state"
                                :data="DepList"
                                :init="dep.name"
                                @checkCell="handleNodeClick"
                                @clear="clearSelect('dep')"
                                @search="searchDep"
                                key="other"></select-tree>
                            <div class="h32"
                                :class="[!firstCreate.state?'no-min':'']"
                                v-else>{{firstCreate.content.storeName}}</div>
                ...
                <el-option class="drop-tree" value="">
                  <el-tree :data="DepList" :props="defaultProps" @node-click="handleNodeClick"></el-tree>
                            ...
                            <el-select :clearable="true"
                                ...
                                <el-option :label="form.inObj"
                                    :value="form.inObjId"
                                    v-if="employeInfo"></el-option>
                                <el-option v-for="(item,index) in EmployeList"
                                    :key="index"
                                    :label="item.name"
                                    :value="item.empId">
                                </el-option>
                        ...
                        <div class="h32"
                            v-else>{{dep.name}}-{{form.inObj}}</div>
                    </div>
                </li>
                <li>
                    <div class="input-group col"
                    ...
                    <div class="input-group col"
                        :class="[!firstCreate.state?'no-max':'']"
                        v-if="billStatus">
                        ...
                        <el-select size="small"
                            ...
                            <el-option v-for="item in account"
                                :key="item.id"
                        ...
                        <div class="h32"
                            ...
                            v-else>
                            {{firstCreate.content.account[0].storeName}}-{{firstCreate.content.account[0].userName}}-{{firstCreate.content.account[0].bankName}}-{{firstCreate.content.account[0].cardNumber}}
                        </div>
                    </div>
                </li>
            </ul>
            <div class="input-group"
                ...
                <div class="flex-box txt-title">
                    ...
                    <el-tooltip content="多种收款方式可通过“添加支付方式”进行录入"
                        placement="top">
                        <p class="tip-message"><i class="iconfont icon-wenhao"></i>填写帮助</p>
                    </el-tooltip>
                </div>
                        ...
                        <div class="message-box flex-box">
                            <section>
                                ...
                                <el-select size="small"
                                    ...
                                    <el-option-group v-for="group in getDir"
                                        :key="group.label"
                                        :label="group.label">
                            ...
                            <section>
                                ...
                                <input type="text"
                                    ...
                                    :disabled="!getPayMethod(item.payMethod)"
                                    placeholder="请输入"
                                    v-model="item.fee"
                  ...
                  <el-option
                    ...
                    {{item.storeName}}-{{item.bankAccountName}}<span style="margin: 0 4px;">{{item.bankBranchName}}</span>{{item.bankCard}}
                        ...
                        <div class="bankCard-list"
                            v-if="billStatus&&payList[index].payMethod!==3">
            ...
            <section>
                ...
                <div class="col-other other-message">
                    <div class="input-group"
                        v-if="billStatus">
                        <p><label class="form-label f14">付款凭证</label><span>（凭证类型：转账汇款电子回单、买卖交易合同、收据、租赁合同、解约协议、定金协议、意向金协议）</span></p>
                        <ul class="upload-list">
                            ...
                            <li v-for="(item,index) in imgList"
                                ...
                                <img :src="item|getSignImage(preloadFiles,_self)"
                                    alt=""
                                    v-if="isPictureFile(item.type)"
                                    height="90px"
                                    :key="item.path"
                                    :width="item.width">
                                <upload-cell :type="item.type"
                                    v-else></upload-cell>
                                <!--<span>{{item.name}}</span>-->
                                ...
                                <p v-show="activeLi===index"
                                    @click.stop="delFile"><i class="iconfont icon-tubiao-6"></i></p>
                            </li>
                        </ul>
        ...
        <p>
            <el-button class="btn-info"
                round
                size="small"
                @click="goCancel">取消</el-button>
            <el-button class="btn-info"
                round
                size="small"
                type="primary"
                @click="goResult"
                v-loading.fullscreen.lock="fullscreenLoading">{{!billStatus?'创建POS收款订单':'录入信息并提交审核'}}
            </el-button>
        </p>
        ...
        <!-- 房源客源弹窗 -->
        <houseGuest :dialogType="addressValCom.type"
...
export default {
    ...
    methods: {
            ...
            this.$ajax
                ...
                .then(res => {
                    ...
                    if (res.status === 200) {
                        if (res.data) {
                            this.billStatus =
                                res.data.account.value === 3 ? false : true;
                            this.hasChose_receiptType.state = true;
                            this.hasChose_receiptType.version =
                                res.data.version;
                        }
                    }
                });
        ...
        getDetails: function(param) {
            this.$ajax.get("/api/payInfo/selectRevDetail", param).then(res => {
                ...
                if (res.status === 200) {
                    ...
                    this.billStatus =
                        res.data.inAccountType === 3 ? false : true;
        ...
        getFiles: function(payload) {
            ...
            this.imgList.forEach(item => {
                if (this.isPictureFile(item.type)) {
                    ...
                    !hasImg &&
                        this.fileSign([].concat(item.path), "preload").then(
                            res => {
        ...
        goResult: function() {
            ...
            //支付信息验证
            if (this.billStatus) {
                let cardListStatus = newPayList.every(
                    item => item.payMethod === 3
                );
                newPayList.forEach((item, index) => {
                    ...
                    if (!cardListStatus && this.cardList[index].cardNumber) {
                        arr.push(
                            this.$tool.checkForm(this.cardList[index], cardRule)
                        );
                    }
                ...
                Promise.all(arr)
                    .then(res => {
                        ...
                        if (total !== parseFloat(payTotal.toFixed(2))) {
                            ...
                            param.outAccount = cardListStatus
                                ? []
                                : [].concat(this.cardList);
                            param.inAccount = [];
                            this.payList.forEach(item => {
                                if (
