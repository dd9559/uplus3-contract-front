<template>
    <el-table class="info-scrollbar" :max-height="tableHeight" border :data="getList" header-row-class-name="theader-bg" style="width: 100%;" @row-dblclick="toDetails">
      ...
      <el-table-column label="合同信息" min-width="200" prop="cityName" :formatter="nullFormatter">
        <template slot-scope="scope">
          <ul class="contract-msg">
            <li>合同:<span @click="toLink(scope.row,'xf')">{{scope.row.cCode}}</span></li>
            <li class="code-paper" v-if="scope.row.cPaperCode">纸质合同编号:<span @click="toLink(scope.row,'xf')">{{scope.row.cPaperCode}}</span></li>
          </ul>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="项目名称" prop="projectName"
                       :formatter="nullFormatter"></el-table-column>
      ...
      <el-table-column min-width="90" label="成交总价" prop="dealPrice" :formatter="nullFormatter"></el-table-column>
      ...
      <el-table-column min-width="80" label="客户姓名" prop="objName" :formatter="nullFormatter"></el-table-column>
      <el-table-column min-width="120" label="产权地址" prop="propertyRightAddr" :formatter="nullFormatter"></el-table-column>
      <el-table-column min-width="80" label="户型" prop="houseType" :formatter="nullFormatter"></el-table-column>
      <el-table-column min-width="90" label="建筑面积（㎡）" prop="square" :formatter="nullFormatter"></el-table-column>
      ...
      <el-table-column label="操作" fixed="right" min-width="120">
        <template slot-scope="scope">
          <div v-if="(scope.row.state.value!==1&&power['sign-xf-cw-rev-edit'].state)||(scope.row.state.value===3&&getUser.user&&(getUser.user.empId===scope.row.auditId))">
            <el-button type="text" @click="cellOpera(scope.row,'edit')" v-if="scope.row.state.value!==1&&power['sign-xf-cw-rev-edit'].state">编辑</el-button>
