  ...
  <div class="view" ref="tableComView">
    ...
    <div class="view-context"
      ...
      <el-table class="info-scrollbar" ref="tableCom" :max-height="tableNumberCom" border :data="list" :key="activeView"
        ...
        <el-table-column min-width="120" :label="getView" prop="payCode"
        ...
        <el-table-column min-width="120" label="物业地址" prop="address"
                         :formatter="nullFormatter"></el-table-column>
        ...
        <el-table-column label="操作" fixed="right" min-width="120" class-name="null-formatter">
          <template slot-scope="scope">
            <el-button type="text" @click="cellOpera(scope.row,'edit')"
                       v-if="(scope.row.payway.value!==4&&scope.row.payStatus.value!==5||scope.row.payway.value===4&&scope.row.billStatus.value!==2)&&scope.row.type===1">编辑
            </el-button>
            <el-button type="text" @click="cellOpera(scope.row)" v-if="scope.row.auditButton">审核</el-button>
            <el-button type="text" @click="cellOpera(scope.row,'del')"
                       v-if="((activeView===1&&scope.row.billStatus&&scope.row.billStatus.value===1)||activeView===2)&&
                       scope.row.isDel===1&&
                       (scope.row.caozuo===1||scope.row.caozuo===2)&&
                       power[activeView===1?'sign-cw-rev-void':'sign-cw-pay-void'].state">作废
            </el-button>
          </template>
        </el-table-column>
      </el-table>
