    ...
    <div class="view-context">
      ...
      <el-table class="info-scrollbar" ref="tableCom" :max-height="tableNumberCom" border :data="list" :key="activeView" style="width: 100%;max-height:500px;" header-row-class-name="theader-bg" @row-dblclick="toDetails">
        ...
        <el-table-column align="center" label="操作" fixed="right" min-width="120">
          <template slot-scope="scope">
            <template v-if="(scope.row.auditButton)||((((activeView===1)&&scope.row.billStatus&&(scope.row.billStatus.value===1))||activeView===2)&&scope.row.isDel===1&&scope.row.caozuo===0&&power[activeView===1?'sign-cw-rev-void':'sign-cw-pay-void'].state)">
              <el-button type="text" @click="cellOpera(scope.row)" v-if="scope.row.auditButton">审核</el-button>
              <el-button type="text" @click="cellOpera(scope.row,'del')" v-if="(((activeView===1&&scope.row.billStatus&&scope.row.billStatus.value===1)||activeView===2)&&scope.row.isDel===1&&scope.row.caozuo===0&&power[activeView===1?'sign-cw-rev-void':'sign-cw-pay-void'].state)">作废</el-button>
            </template>
            <span v-else>--</span>
          </template>
        </el-table-column>
