        ...
        <el-table-column fixed="right" label="操作" min-width="120" class-name="null-formatter operation-btns">
          <template slot-scope="scope">
            <el-button type="text" @click="btnOpera(scope.row,3)"
              ...
              开票
            </el-button><el-button type="text" @click="btnOpera(scope.row,1)" v-if="scope.row.payway&&scope.row.payStatus&&(scope.row.payway.value!==4||scope.row.payway.value===4&&scope.row.billStatus.value!==2)&&scope.row.payStatus.value!==5&&scope.row.type===1&&scope.row.edit===1">编辑</el-button><template
