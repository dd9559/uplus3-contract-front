    ...
    <div class="view-context" ref="box">
      ...
      <el-table ref="tableCom" :max-height="tableNumberCom" border :data="list" class="info-scrollbar" style="width: 100%" header-row-class-name="theader-bg" @row-dblclick="toDetails">
        ...
        <el-table-column fixed="right" align="center" label="操作" min-width="160">
          <template slot-scope="scope">
            <template v-if="scope.row.type===1&&scope.row.billStatus&&(scope.row.billStatus.value===1)&&(power['sign-cw-debt-invoice'].state||((scope.row.caozuo===1||scope.row.caozuo===0)&&(power['sign-cw-debt-edit'].state||power['sign-cw-debt-void'].state)))">
