    ...
    <ScreeningTop @propResetFormFn="reset" @propQueryFn="getData('search')">
      <div class="content">
        ...
        <div class="input-group">
          ...
          <el-select :clearable="true" class="margin-left" size="small" v-loadmore="moreEmploye"
                     v-model="searchForm.empId" @change="handleEmpNodeClick" placeholder="请选择">
        ...
        <el-table-column label="合同信息" min-width="200" prop="cityName" :formatter="nullFormatter">
          <template slot-scope="scope">
            <ul class="contract-msglist">
              ...
              <li class="code-paper" v-if="scope.row.recordType.value===2">纸质合同编号:<span
                @click="toLink(scope.row,'cont')">{{scope.row.paperCode|getLabel}}</span></li>
        ...
        <el-table-column fixed="right" label="操作" min-width="120" class-name="null-formatter operation-btns">
          <template slot-scope="scope">
            <el-button type="text" @click="btnOpera(scope.row,3)"
              ...
              开票
            </el-button><el-button type="text" @click="btnOpera(scope.row,1)" v-if="scope.row.payway&&scope.row.payStatus&&(scope.row.payway.value!==4||scope.row.payway.value===4&&scope.row.billStatus.value!==2)&&scope.row.payStatus.value!==5&&(scope.row.type===1||scope.row.type===8)&&scope.row.edit===1&&power['sign-cw-debt-update'].state">编辑</el-button><template
            ...
            </template><div v-if="power['sign-cw-bill-invoice'].state&&scope.row.billStatus&&scope.row.billStatus.value===2">
