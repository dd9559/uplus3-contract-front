    ...
    <div ref="tableComView">
        ...
        <ScreeningTop @propQueryFn="queryFn"
            ...
            <el-form :inline="true"
                ...
                <el-form-item label="关键字"
                    prop="search">
                    <el-tooltip content="合同编号/纸质合同编号/物业地址/业主/客户/房产证号/手机号"
                        placement="top">
                        <el-input class="w200"
                ...
                <div class="in-block">
                    <el-form-item prop="region"
                        class="mr">
                        <el-select v-model="propForm.region"
                            @change="receivingdateChangeFn"
                            class="w110">
                            <el-option v-for="item in rules.region"
                                :key="item.value"
                                :label="item.label"
                                :value="item.value"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item v-show="propForm.region === RECEIVINGDATE.end"
                        class="mr"
                        prop="steps">
                        <el-select v-model="propForm.steps"
                            placeholder="交易步骤"
                            class="w110">
                    ...
                    <el-form-item prop="dateMo">
                        <el-date-picker v-model="propForm.dateMo"
                            class="w284"
                            type="daterange"
                            range-separator="至"
                ...
                <div class="in-block">
                    ...
                    <el-form-item prop="stepsMo"
                        ...
                        class="mr">
                        <!-- @change="rulesStepsFn" -->
                        <el-select v-model="propForm.stepsMo"
                            class="w110"
                            placeholder="状态">
                            <el-option v-for="item in rules.stepsMo"
                ...
                <el-form-item label="交易流程"
                    prop="process">
                    <el-select v-model="propForm.process"
                        class="w270">
                        <el-option v-for="item in rules.process"
                            :key="'jy'+item.id"
                            :label="item.name"
                            :value="item.id"></el-option>
                    </el-select>
                ...
                <el-form-item label="是否超时"
                    prop="timeout">
                    <el-select v-model="propForm.timeout"
                        class="w110">
                        <el-option v-for="item in rules.timeout"
                ...
                <el-form-item label="贷款银行"
                    prop="bank">
                    <el-select v-model="propForm.bank"
                        :remote-method="remoteMethodFn"
                        remote
                        clearable
                        filterable
                        class="w262">
                        <el-option v-for="item in rules.bank"
                ...
                <el-form-item label="数据范围"
                    prop="range">
                    <el-select v-model="propForm.range"
                        class="w180">
                        <el-option v-for="item in rules.range"
                            :key="'range'+item.key"
                            :label="item.value"
                            :value="item.key"></el-option>
                    </el-select>
                ...
                <el-form-item label="变更/解约"
                    prop="termination">
                    <el-select v-model="propForm.termination"
                        class="w120">
                        <el-option v-for="item in rules.termination"
                ...
                <el-form-item label="后期状态"
                    prop="lateState">
                    <el-select v-model="propForm.lateState"
                        class="w100">
                        <el-option v-for="item in rules.lateState"
                ...
                <div class="in-block">
                    ...
                    <el-form-item prop="departmentMo">
                        <el-select v-model="propForm.departmentMo"
                ...
                <el-form-item label="收佣状态"
                    prop="commission">
                    <el-select v-model="propForm.commission"
                        class="w100">
                        <el-option v-for="item in rules.commission"
                ...
                <el-form-item label="合作方式"
                    prop="depAttr">
                    <el-select v-model="propForm.depAttr"
                        class="w100"
                        :clearable="true">
                        <el-option v-for="item in rules.depAttr"
                            :key="'depAttr'+item.key"
                            :label="item.value"
                            :value="item.key"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="产权地址区域"
                    prop="areaName">
                    <el-select v-model="propForm.areaName"
                        class="w134"
                        :clearable="true">
                ...
                <el-form-item label="签约方式">
                    <el-select v-model="propForm.recordType"
                        class="w120"
                        :clearable="true">
                        <el-option v-for="item in dictionary['64']"
                            :key="item.key"
                            :label="item.value"
                            :value="item.key">
                        </el-option>
                ...
                <div class="in-block">
                    <el-form-item prop="regionTime"
                        class="mr">
                        <el-select v-model="propForm.regionTime"
                            @change="regionTimeChangeFn"
                            class="w110">
                            <el-option v-for="item in rules.regionTime"
                                :key="item.value"
                                :label="item.label"
                                :value="item.value"></el-option>
                        </el-select>
                    ...
                    <el-form-item prop="dateTime">
                        <el-date-picker v-model="propForm.dateTime"
                            class="w284"
                            type="daterange"
                            range-separator="至"
                            start-placeholder="开始日期"
        ...
        <div class="paper-table-box">
            <div class="paper-set-tit">
                <div class="paper-tit-fl"
                    v-if="power['sign-qh-mgr-export'].state"><i class="iconfont icon-tubiao-11 mr-10 font-cl1"></i>数据列表</div>
                <p>
                    <el-button class="btn-info"
                        ...
                        size="small"
                        @click="getExcel"
                        v-if="power['sign-qh-mgr-export'].state">导出</el-button>
                </p>
            </div>
            <el-table border
                ref="tableCom"
                ...
                class="paper-table">
                <el-table-column :formatter="nullFormatterData"
                    prop="code"
                    label="合同信息"
                    min-width="150">
                    <template slot-scope="scope">
                        <p>
                            ...
                            <span class="blue"
                                ...
                                style="cursor:pointer;">{{scope.row.code}}</span>
                        </p>
                        <p v-if="scope.row.recordType.value===2">
                            纸质合同编号:
                            <span class="blue"
                                @click="contractFn(scope.row)"
                                style="cursor:pointer;">{{scope.row.pCode}}</span>
                        </p>
                        <p>{{scope.row.remarkAfter.value > 0 ?scope.row.remarkAfter.label:''}}</p>
                    </template>
                </el-table-column>
                <el-table-column prop="recordType.label"
                    label="签约方式"
                    min-width="60"></el-table-column>
                <el-table-column :formatter="nullFormatterData"
                    prop="statusLaterStage.label"
                    label="后期状态"
                    min-width="60">
                </el-table-column>
                ...
                <el-table-column :formatter="nullFormatterData"
                    ...
                    min-width="100">
                    <template slot-scope="scope">
                        {{dateFormat(scope.row.transferTime)}}
                    </template>
                </el-table-column>
                <el-table-column :formatter="nullFormatterData"
                    prop="a10"
                    ...
                    <template slot-scope="scope">
                        <template v-if="scope.row.statusLaterStage.label === STATE.start">
                            <el-button class="blue"
                                type="text"
                                @click="progressFn(scope.row)">查看交易流程</el-button>
                        </template>
                        <template v-else>
                            <p class="steps-p">
                                <span class="steps-span">当前步骤：</span>
                                <span v-if="scope.row.stepInstanceName==='-'"
                                    class="steps-line">-</span>
                                <el-tooltip v-else
                                    class="item"
                                    effect="dark"
                                    ...
                                    <el-button class="blue"
                                        type="text"
                                        @click="transactionFn(scope.row,1)">{{scope.row.stepInstanceName}}</el-button>
                                </el-tooltip>
                            </p>
        ...
        <div class="pagination"
            ...
            <el-pagination :current-page="tableData.pageNum"
        ...
        <el-dialog title="后期进度"
            ...
            <LayerScrollAuto>
                <div class="layer-progress">
                    <ul class="ul"
                        :class="layerBtn?'ul-line':''">
                        <li class="mr-30"
                            v-if="layerBtn"
                            v-show="layerBtnShowFn(power['sign-qh-mgr-jd-modify'].state,power['sign-qh-mgr-jd-smgr'].state)">
                            <el-button v-if="power['sign-qh-mgr-jd-modify'].state"
                            ...
                            <el-button v-if="power['sign-qh-mgr-jd-smgr'].state"
                    ...
                    <el-table border
                        ...
                        <el-table-column label="操作"
                            ...
                            align="center">
                            <template slot-scope="scope"
                                v-if="layerShowData.statusChange.value!=2">
                                <template v-if="scope.row.stepState.value === OPERATION.start">
                                    <el-button class="blue"
                                        type="text"
                                        @click="operationFn(scope.row.id)">查看</el-button>
                                </template>
                                <template v-else-if="scope.row.stepState.value === OPERATION.backlog && layerShowData.statusLaterStage.label !== STATE.start">
                                    <el-button class="blue"
                                        type="text"
                                        @click="transactionFn(scope.row)">办理</el-button>
                                    <el-button class="blue"
                                        type="text"
                                        v-if="scope.$index !== tableProgress.length-1 && power['sign-qh-mgr-jd-move'].state"
                                        @click="downFn(scope)">下</el-button>
                                </template>
                                <template v-else-if="scope.row.stepState.value === OPERATION.sure && layerShowData.statusLaterStage.label !== STATE.start">
                                    <el-button class="blue"
                                        type="text"
                                        @click="sureFn(scope.row)">{{showSeeFn('确认',scope.row)}}</el-button>
                                </template>
                                <template v-else-if="scope.row.stepState.value === OPERATION.not && layerShowData.statusLaterStage.label !== STATE.start">
                                    <el-button class="blue"
                                        v-if="isUpBtnFn(scope.$index) && power['sign-qh-mgr-jd-move'].state"
                                        type="text"
                                        @click="upFn(scope)">上</el-button>
                                    <el-button v-if="scope.$index !== tableProgress.length-1 && power['sign-qh-mgr-jd-move'].state"
                                        class="blue"
                                        type="text"
                                        @click="downFn(scope)">下</el-button>
                                ...
                                <template v-else-if="scope.row.stepState.value === OPERATION.amend">
                                    <el-button class="blue"
                                        type="text"
                                        @click="amendFn(scope.row)">{{showSeeFn('修改',scope.row)}}</el-button>
                                </template>
        ...
        <el-dialog title="选择交易流程"
            ...
            <div class="replace-box"
                v-loading="loadingReplace">
                <div>
                    <div class="tit">交易流程</div>
                    <ul class="ul">
                        <li v-for="item in replaceData.tit"
                ...
                <div>
                    ...
                    <ul class="ul-box pr">
                        <li v-for="item in replaceData.children"
                            :key="'jybz'+item.id">
                        ...
                        <li v-show="replaceData.children.length === 0"
                            class="no-data">
            ...
            <span slot="footer">
                <el-button class="paper-btn"
                    type
                    size="small"
                    @click="replaceCloseFn"
                    round>返回</el-button>
                <el-button class="paper-btn paper-btn-blue"
                    type="primary"
                    size="small"
                    @click="replaceBtnFn"
                    round>确定</el-button>
        ...
        <!-- 办理 -->
        <el-dialog :title="stepsData.tit"
            :close-on-click-modal="$tool.closeOnClickModal"
            :close-on-press-escape="$tool.closeOnClickModal"
            :visible.sync="stepsData.show"
            width="740px"
                ...
                <div class="steps-from">
                    ...
                    <div class="select-title">
                        <ul class="select-option"
                            v-if="stepReportData.length>0">
                            <li v-for="item in optionTab"
                                :key="item.id"
                                :class="[activeItem===item.id?'active':'']"
                                @click="changeTab(item)">{{item.name}}</li>
                        </ul>
                        <div class="stepsData-tit"
                            v-else>{{stepsData.tit}}</div>
                    </div>
                    <div class="handle-choose"
                        v-show="(stepsData.tit === STEPS.start || stepsData.tit === STEPS.affirm || stepsData.tit === stepReportTit)&&activeItem===1">
                        <span>是否完成：</span>
                        <el-radio-group v-model="isHandle"
                            @change="handleChange">
                            <el-radio label="1">是，现在办结</el-radio>
                            <el-radio label="0">否，写跟进日志</el-radio>
                        </el-radio-group>
                    </div>
                    <el-form ref="stepsFrom"
                        :model="stepsFrom"
                        ...
                        <el-form-item v-for="(item,index) in stepsFrom.list"
                            :prop="'list.' + index + '.val'"
                                ...
                                <template v-if="stepsData.tit !== STEPS.end">
                                    ...
                                    <template v-if="item.type === STEPSINPUT.num">
                                        <el-input v-model="item.val"
                                            ...
                                            @focus="inputFocusFn"></el-input>
                                    </template>
                                    <template v-else-if="item.type === STEPSINPUT.time">
                                        <el-date-picker v-model="item.val"
                                            size="small"
                                            type="date"
                                            class="w160"
                                        ...
                                        <ul class="steps-img">
                                            <el-tooltip class="item"
                                                ...
                                                <li @click="previewPhoto(item.val,n)">
                                                    <i @click.stop="clearFn(index,n)"
                                                        class="iconfont icon-tubiao-6"></i>
                                                    <img class="suolue-img"
                    ...
                    <el-form v-show="isHandle === '0'&&activeItem===1"
                        label-width="148px"
                        :model="stepReportFrom"
                        :rules="stepReportRules"
                        ref="stepReportFrom">
                        ...
                        <el-form-item label="跟进时间："
                            prop="reportingDate">
                            <el-date-picker v-model="stepReportFrom.reportingDate"
                                size="small"
                                type="date"
                                class="w160"
                                :picker-options="pickerOptions"
                        ...
                        <el-form-item label="日志内容："
                            class="step-report-log">
                            <el-input v-model.trim="stepReportFrom.reportingRemake"
                                type="textarea"
                                resize="none"
                                :maxlength="invalidMax"
                                class="steps-input">
                    ...
                    <div class="paper-table step-report-table"
                        v-if="stepReportData.length>0&&activeItem===2">
                        <!-- <div class="title">跟进记录：</div> -->
                        <el-table border
                            :data="stepReportData">
                            <el-table-column align="center"
                                label="跟进人"
                                prop="reportingtor"></el-table-column>
                            <el-table-column align="center"
                                label="跟进时间">
                                <template slot-scope="scope">
                            ...
                            <el-table-column align="center"
                                ...
                                :formatter="nullFormatterData"></el-table-column>
                        </el-table>
                    </div>
                    <!-- 预览 -->
        ...
        <el-dialog title="调整步骤"
            ...
            <LayerScrollAuto>
                <div class="layer-chekbox-box"
                    ...
                    <el-table :data="adjustData"
                        class="paper-table"
                        border>
                        <el-table-column class="layer-all-chekbox"
                            align="center"
                            width="250px"
                            label="标题">
                            <template slot-scope="scope">
                                <p class="check-all">
                                    <el-checkbox class="check-break"
                        ...
                        <el-table-column align="center"
                            label="内容">
                            <template slot-scope="scope">
                                <el-checkbox-group class="layer-checkbox-p"
                                    v-model="scope.row.list"
                                    @change="adjustChange(scope.$index,$event)">
                                    <p v-for="item in scope.row.stepsList"
                                        :key="item.name">
                                        <el-checkbox class="check-break"
...
export default {
    ...
    methods: {
        ...
        isUpBtnFn(index) {
            ...
            if (
                this.tableProgress[index - 1].stepState.value === OPERATION.sure
            ) {
                return false;
            } else {
                return true;
                    ...
                    if (res.status === 200) {
                        ...
                        if (tit == "查看") {
                            ...
                            if (preloadList.length) {
                                this.fileSign(preloadList, "preload").then(
                                    res => {
        ...
        replaceFn() {
            ...
            this.$ajax
                .get("/api/postSigning/getStepList", {
        ...
        managementFn() {
            ...
            this.$ajax
                .post("/api/flowmanage/selectTypeStepsList", {
                    cityId: this.cityId
                })
                .then(res => {
                    res = res.data;
        ...
        replaceBtnFn() {
            ...
            if (this.replaceData.transFlowCode === this.replaceData.index) {
                ...
                this.$ajax
                    .post("/api/postSigning/updateTransFlow", {
        ...
        replaceTabFn(i) {
            ...
            if (i === this.replaceData.transFlowCode) {
                ...
                this.$ajax
                    .post("/api/flowmanage/selectFlowStepsList", {
                        flowId: i
                    })
                    .then(res => {
                        res = res.data;
        ...
        stepReportFn() {
            this.$refs["stepReportFrom"].validate(valid => {
                if (valid) {
                    ...
                    this.$ajax
                        .postJSON("/api/postSigning/addStepReporting", param)
                        .then(res => {
                            res = res.data;
                            if (res.status === 200) {
                                this.successMeFn(res.message);
        ...
        confirmStepFn(url = "/api/postSigning/confirmStep") {
            ...
            this.$refs["stepsFrom"].validate(valid => {
                if (valid) {
                    ...
                    // 添加备注和时间
                    let handleDatetime = this.dateFormat(
                        transAtepsAttach[transAtepsAttach.length - 2].val
                    );
                    let remarks =
                        transAtepsAttach[transAtepsAttach.length - 1].val;
                    // 删除多余的数组
        ...
        imgBtnFn(e) {
            ...
            this.stepsFrom.list[index].val = this.stepsFrom.list[
                index
            ].val.concat(arr);
        ...
        adjustBtnFn() {
            ...
            if (this.copyAdjustData === arr.join()) {
                ...
                // 修改了
                this.$ajax
                    .postJSON("/api/postSigning/updateStepInstance", {
                        contractCode: this.layerShowData.id,
                        steps
                    })
                    .then(res => {
        ...
        this.$nextTick(() => {
            ...
            if (res && res.route === this.$route.path) {
                ...
                let session = JSON.parse(
                    sessionStorage.getItem("sessionQuery")
                );
                let query = session.query;
                this.propForm = {
                    search: query.keyword,
                    dateMo: query.receiveTimeStar
                        ? [query.receiveTimeStar, query.receiveTimeEnd]
                        : query.handleTimeStar
                        ? [query.handleTimeStar, query.handleTimeEnd]
                        : "",
                    dateMoB: "",
