  ...
  <div ref="tableComView">
      ...
      <el-form :inline="true" ref="propForm" :model="propForm" class="prop-form" size="small">
        ...
        <div class="in-block">
          <el-form-item prop="region" class="mr">
            <el-select v-model="propForm.region" @change="receivingdateChangeFn" class="w110">
              <el-option
          ...
          <el-form-item v-show="propForm.region === RECEIVINGDATE.end" class="mr" prop="steps">
            <el-select v-model="propForm.steps" placeholder="交易步骤" class="w110">
              <el-option
                v-for="item in rulesSteps"
        ...
        <div class="in-block">
          <el-form-item prop="stepsMo" label="交易步骤" class="mr">
            <!-- @change="rulesStepsFn" -->
            <el-select v-model="propForm.stepsMo" class="w110" placeholder="状态">
              <el-option
        ...
        <el-form-item label="交易流程" prop="process">
          <el-select v-model="propForm.process" class="w270">
            <el-option
        ...
        <el-form-item label="是否超时" prop="timeout">
          <el-select v-model="propForm.timeout" class="w110">
            <el-option
        ...
        <el-form-item label="数据范围" prop="range">
          <el-select v-model="propForm.range" class="w180">
            <el-option
        ...
        <el-form-item label="变更/解约" prop="termination">
          <el-select v-model="propForm.termination" class="w120">
            <el-option
        ...
        <el-form-item label="后期状态" prop="lateState">
          <el-select v-model="propForm.lateState" class="w100">
            <el-option
        ...
        <el-form-item label="收佣状态" prop="commission">
          <el-select v-model="propForm.commission" class="w100">
            <el-option
    ...
    <div class="paper-table-box">
      <div class="paper-set-tit">
        <div class="paper-tit-fl" v-if="power['sign-qh-mgr-export'].state">
      ...
      <el-table
        ...
        class="paper-table"
      >
        <el-table-column :formatter="nullFormatterData" prop="code" label="合同信息" min-width="150">
        ...
        <el-table-column
          ...
          min-width="60"
        ></el-table-column>
        <el-table-column label="变更/解约" min-width="60">
          <template slot-scope="scope">{{statusChangeFn(scope.row.statusChange)}}</template>
        </el-table-column>
        <el-table-column
          :formatter="nullFormatterData"
          prop="propertyAddr"
              ...
              <p class="steps-p">
                ...
                >
                  <el-button
                    class="blue"
                    type="text"
                    @click="transactionFn(scope.row,1)"
                  >{{scope.row.stepInstanceName}}</el-button>
                </el-tooltip>
              </p>
    ...
    >
      <LayerScrollAuto>
        <div class="layer-progress">
          ...
          >
            <el-table-column min-width="120px" label="步骤类型" align="center">
              <template slot-scope="scope">
                <span
            ...
            <el-table-column min-width="120px" label="步骤名称" align="center">
              <template slot-scope="scope">
                <span
                  :class="scope.row.isOvertime.value === ISOVERTIME?'red':'cl-2'"
            ...
            <el-table-column label="确定日期" min-width="90px" align="center">
              <template slot-scope="scope">
                <span
                  :class="scope.row.isOvertime.value === ISOVERTIME?'red':'cl-2'"
            ...
            <el-table-column label="操作" min-width="120px" align="center">
              <template slot-scope="scope" v-if="layerShowData.statusChange.value!=2">
                ...
                <template
                  v-else-if="scope.row.stepState.value === OPERATION.backlog && layerShowData.statusLaterStage.label !== STATE.start"
                >
                  <el-button class="blue" type="text" @click="transactionFn(scope.row)">办理</el-button>
                  <el-button
                    class="blue"
                    type="text"
                ...
                <template
                  v-else-if="scope.row.stepState.value === OPERATION.sure && layerShowData.statusLaterStage.label !== STATE.start"
                >
                  <el-button
                    class="blue"
                    type="text"
                    @click="sureFn(scope.row)"
                  >{{showSeeFn('确认',scope.row)}}</el-button>
                </template>
                <template
                  v-else-if="scope.row.stepState.value === OPERATION.not && layerShowData.statusLaterStage.label !== STATE.start"
                >
                  <el-button
                    class="blue"
                ...
                <template v-else-if="scope.row.stepState.value === OPERATION.amend">
                  <el-button
                    class="blue"
                    type="text"
                    @click="amendFn(scope.row)"
                  >{{showSeeFn('修改',scope.row)}}</el-button>
                </template>
    ...
    >
      <div class="replace-box" v-loading="loadingReplace">
        ...
        <div>
          ...
          <ul class="ul-box pr">
            <li v-for="item in replaceData.children" :key="'jybz'+item.id">
              <span
                v-show="replaceData.index === replaceData.transFlowCode"
              >{{item.transactionSteps}}</span>
              <span v-show="replaceData.index !== replaceData.transFlowCode">{{item.stepsName}}</span>
            </li>
    ...
    >
      <LayerScrollAuto>
        <div class="steps-from">
              ...
              <div class="steps-postion-b">
                ...
                <template v-if="stepsData.tit !== STEPS.end">
                  <template v-if="item.type === STEPSINPUT.start">
                    <el-input v-model="item.val" @input="textChangeFn(index,$event)" size="small"></el-input>
                  </template>
                  ...
                  <template
                    v-else-if="item.type === STEPSINPUT.img || item.type === STEPSINPUT.mp4 || item.type === STEPSINPUT.pdf || item.type === STEPSINPUT.excel || item.type === STEPSINPUT.word"
                  >
                    <div>
                      <fileUp
                        v-if="stepsData.show"
                      ...
                      >
                        <el-button
                          ...
                          type="primary"
                          size="small"
                        >{{stepsTypeImg(item.type,2)}}</el-button>
                      </fileUp>
                    </div>
                    <ul class="steps-img">
                      <el-tooltip
          ...
          <el-form
            v-show="isHandle === '0'&&activeItem===1"
            label-width="148px"
    ...
    >
      <LayerScrollAuto>
        <div class="layer-chekbox-box" v-loading="loadingAdjust">
          <el-table :data="adjustData" class="paper-table" border>
            <el-table-column class="layer-all-chekbox" align="center" width="250px" label="标题">
              <template slot-scope="scope">
                <p class="check-all">
                  <el-checkbox
                    class="check-break"
                    v-model="scope.row.bool"
                    @change="adjustAllChange(scope.$index,$event)"
                  >{{scope.row.typeName}}</el-checkbox>
                </p>
              </template>
            </el-table-column>
            <el-table-column align="center" label="内容">
              <template slot-scope="scope">
                ...
                >
                  <p v-for="item in scope.row.stepsList" :key="item.name">
                    <el-checkbox
                      class="check-break"
...
export default {
  ...
  methods: {
              ...
              e.stepsList.map(i => {
                this.tableProgress.forEach(t => {
                  if (t.transactionStepsCode === i.id) {
                    ...
                    arr2.push(i.id);
                    if (
                      t.stepState.value !== OPERATION.not &&
                      t.stepState.value !== OPERATION.backlog
                    ) {
                      i.disabled = true;
    ...
    imgBtnFn(e) {
      ...
      this.stepsFrom.list[index].val = this.stepsFrom.list[index].val.concat(
        arr
      );
