    ...
    <div class="contract-list">
      <div class="listTitle">
        ...
        <div class="float-right">
          <el-button
            class="btn-info"
            v-if="power['sign-ht-qhsh-export'].state"
            round
            type="primary"
            size="small"
            @click="getExcel"
      ...
      >
          ...
          <template slot-scope="scope">
            <div class="contract_msg">
              ...
              <ul class="contract-msglist">
                ...
                <li v-if="scope.row.honseCode">
                  房源：
                  <span>{{scope.row.honseCode}}</span>
                  {{scope.row.showOwnerName}}
                </li>
                <li>
                  客源：
                  <span>{{scope.row.guestCode}}</span>
          ...
          <template slot-scope="scope">
            <div v-if="scope.row.contTypes">
              ...
              <span
                ...
                :key="item.key"
                v-if="scope.row.contType.value===1&&item.key===scope.row.timeUnit"
              >/ {{item.value}}</span>
            </div>
            <span v-else>-</span>
          </template>
          ...
          <template slot-scope="scope">
            <template v-if="scope.row.state">
              ...
              <el-tooltip
                ...
                v-if="scope.row.state.value===2&&scope.row.remarks"
                effect="dark"
                placement="top"
              >
        ...
        <el-table-column label="审核时间" min-width="110">
          <template slot-scope="scope">
            <span
              v-if="scope.row.updateTime&&scope.row.updateTime!='-'"
            >{{Number(scope.row.updateTime)|timeFormat_hm}}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column label="当前审核人" min-width="120">
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              v-if="getUserMsg&&(scope.row.currentReviewerId===getUserMsg.empId||scope.row.previousreviewerId===getUserMsg.empId)&&scope.row.state.value===0"
              @click="choseCheckPerson(scope.row,scope.row.previousreviewerId===getUserMsg.empId?1:2)"
            >{{getUserMsg&&getUserMsg.empId===scope.row.currentReviewerId?'转交审核人':'设置审核人'}}</el-button>
          </template>
        </el-table-column>
        <el-table-column label="下一步审核人" min-width="120">
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              v-if="getUserMsg&&(scope.row.nextReviewerId!==0&&scope.row.currentReviewerId===getUserMsg.empId&&scope.row.state&&scope.row.state.value===0)"
              @click="choseCheckPerson(scope.row,3)"
              :class="{'error_':scope.row.nextReviewerId===0}"
            >设置审核人</el-button>
          </template>
        </el-table-column>
        <el-table-column label="操作" min-width="90" fixed="right" class-name="null-formatter">
          <template slot-scope="scope">
            <div
              style="color:red"
              v-if="scope.row.state&&scope.row.state.value===0&&scope.row.currentReviewerId>0&&getUserMsg&&scope.row.currentReviewerId!==getUserMsg.empId"
            >{{scope.row.currentReviewer}}正在审核</div>
            <div
              class="btn"
              v-if="scope.row.state&&scope.row.state.value===0&&((scope.row.currentReviewerId===getUserMsg.empId)||((!(scope.row.currentReviewerId>0))&&getUserMsg&&scope.row.grabDept))"
...
export default {
  ...
  methods: {
    ...
    toDetail(value) {
      // 验证是否有合同详情查看权限
      this.$ajax
        .get("/api/contract/isDetailAuth", { contId: value.cid })
        .then((res) => {
          res = res.data;
          if (res.status === 200) {
            if (res.data) {
    ...
    combineList() {
      ...
      arr.forEach((element, index) => {
        ...
        if (element.contTypes === 0) {
          ...
          element.distributableAchievement =
            element.entrustCont.tradeFeeCommission; //可分配业绩
