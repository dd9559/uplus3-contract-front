      ...
      <el-table :data="tableData.list" ref="tableCom" :max-height="tableNumberCom" style="width: 100%" v-loading="loadingTable" @row-dblclick='toDetail' border>
        ...
        <el-table-column align="center" label="当前审核人" min-width="140">
          <template slot-scope="scope">
            ...
            <el-button class="btn-text-info" type="text" v-if="userMsg&&(scope.row.preAuditId === userMsg.empId || scope.row.checkby === userMsg.empId)&&scope.row.checkState&&scope.row.checkState===0" @click="choseCheckPerson(scope.row.checkId,'init')">转交审核人</el-button>
          </template>
        </el-table-column>
        <el-table-column align="center" label="下一步审核人" min-width="140">
          <template slot-scope="scope">
            ...
            <el-button class="btn-text-info color-red" type="text" v-if="userMsg&&(scope.row.checkby === userMsg.empId)&&scope.row.checkState&&scope.row.checkState===0" @click="choseCheckPerson(scope.row.checkId,'set')">设置审核人</el-button>
          </template>
        </el-table-column>
