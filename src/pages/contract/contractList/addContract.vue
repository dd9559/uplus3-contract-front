  ...
  <div class="view-container">
    <div v-if="contVersion===2">
      ...
      >
        ...
        <div class="contractMsg">
          <p>
            ...
            <span v-if="isDeal" class="toCommission">
              <span
                class="attention iconfont icon-tubiao-10"
                :class="{'attention_':isToCommission}"
              ></span>
              ...
              <span v-if="isToCommission && false">转佣金额（元）：</span>
            </span>
          </p>
          ...
          <div class="form-content">
            <el-form-item label="房源编号：" class="width-250" :class="{'form-label':type===1}">
              <span
                ...
                v-if="(sourceBtnCheck||canInput||!offLine)||(isDeal&&isDeal==1)"
              >{{contractForm.houseinfoCode?contractForm.houseinfoCode:'请选择房源'}}</span>
            ...
            >
              ...
              >{{contractForm.houseInfo.EstateName.replace(/\s/g,"")+' '+contractForm.houseInfo.BuildingName.replace(/\s/g,"")+contractForm.houseInfo.Unit.replace(/\s/g,"")+contractForm.houseInfo.RoomNo.replace(/\s/g,"")}}</span>
            ...
            >
              ...
              >{{contractForm.houseInfo.CompleteYear}}</span>
            ...
            <el-form-item label="业主信息：" class="form-label" style="padding-left:18px">
              <ul class="peopleMsg">
                <li v-for="(item,index) in ownerList" :key="index">
                  ...
                  <span
                    @click.stop="delPeople(index,'owner')"
          ...
          <div class="form-content">
            <el-form-item label="客源编号：" class="width-250" :class="{'form-label':type===1}">
              <span
                ...
                v-if="(sourceBtnCheck||canInput||!offLine)||(isDeal&&isDeal==1)"
              >{{contractForm.guestinfoCode?contractForm.guestinfoCode:'请选择客源'}}</span>
            ...
            <el-form-item label="客户信息：" class="form-label" style="padding-left:18px">
              <ul class="peopleMsg">
                <li v-for="(item,index) in guestList" :key="index">
                  ...
                  <span
                    @click.stop="delPeople(index,'guest')"
        ...
        <!-- 合同备注 -->
        <div class="houseMsg">
          <p @click="showRemarkTab" class="thirdParty">
            备注栏
            <span class="attention iconfont icon-tubiao-10" :class="{'attention_':showRemark}"></span>
          </p>
        ...
        <!-- 三方合作 -->
        <div class="houseMsg">
          <p @click="toCooperation" class="thirdParty">
            三方合作
            <span class="attention iconfont icon-tubiao-10" :class="{'attention_':cooperation}"></span>
          </p>
          <div class="cooperation" v-show="cooperation">
            <div>
              ...
              <el-form-item label="类型：" class="width-250">
                <el-select
                  v-model="contractForm.otherCooperationInfo.type"
                  :disabled="canInput"
                  placeholder="请选择"
                  style="width:140px"
                >
              ...
              <el-form-item label="联系方式：" class="width-250">
                <el-input
                  v-model="contractForm.otherCooperationInfo.mobile"
                  :disabled="canInput"
                  type="tel"
                  placeholder="请输入手机号"
                  style="width:140px"
                  @input="verifyMobile_(contractForm.otherCooperationInfo.mobile)"
                ></el-input>
              </el-form-item>
              <el-form-item label="身份证号：" style="width:310px;text-align:right">
                <el-input
                  v-model="contractForm.otherCooperationInfo.identifyCode"
                  :disabled="canInput"
                  maxlength="18"
                  placeholder="请输入身份证号"
                  @input="verifyIdcard(contractForm.otherCooperationInfo.identifyCode,2)"
                ></el-input>
              </el-form-item>
              <br />
      ...
      <div class="btn">
        <div>
          <div v-if="type===2">
            <p>
              <span>录入时间：</span>
              {{contractForm.createTime|formatTime}}
            </p>
            <p>
              <span>录入人：</span>
              {{contractForm.recordDeptName}}-{{contractForm.recordName}}
            </p>
            <p>
              <span>最后修改：</span>
              {{contractForm.updateTime|formatTime}}
            </p>
          </div>
        </div>
        <div>
          <el-button
            ...
            @click="isSave(0)"
            :disabled="canClick"
          >{{canInput?"保存":"保存并进入下一步"}}</el-button>
        </div>
      </div>
      <!-- 房源客源弹窗 -->
      <houseGuest
      ...
      >
        <div class="warning-box">
          ...
          <ul>
            ...
            >{{item.name}}：{{item.encryptionCode}}</li>
            <li
      ...
      <!-- 分成人信息弹窗 -->
      <el-dialog
        :title="dialogType==='house'?'当前房源分成人':'当前客源分成人'"
        ...
        <div class="agentsDialog">
          ...
          <ul>
            <li
              ...
              :key="index"
              :title="`${item.roleText} ${item.empName}·${item.deptName}`"
            >
        ...
        .then(() => {
          if (
            ...
            if (this.contractForm.dealPrice > 0) {
                ...
                for (var i = 0; i < ownerArr.length; i++) {
                  ...
                  if (element.name) {
                    if (element.name.replace(/\s/g, "")) {
                      ...
                      if (
                        element.name.indexOf("先生") === -1 &&
                        element.name.indexOf("女士") === -1
                      ) {
                        if (element.encryptionMobile.length === 11 || true) {
                          let reg = /^1[0-9]{10}$/; //手机号正则
                          let reg_ = /^0\d{2,3}\-?\d{7,8}$/; //固话正则
                          if (
                            reg.test(element.encryptionMobile) ||
                            reg_.test(element.encryptionMobile)
                          ) {
                            if (element.relation) {
                              if (
                                ...
                                if (
                                  ...
                                  if (
                                    element.encryptionCode.replace(/\s/g, "")
                                  ) {
                                    // if(this.contractForm.type===1){
                                    ...
                                    if (
                                      ...
                                      (element.cardType === 2 &&
                                        element.encryptionCode.length <= 30) ||
                                      (element.cardType === 3 &&
                                        element.encryptionCode.length <= 20) ||
                                      (element.cardType === 4 &&
                                        element.encryptionCode.length <= 10)
                                    ) {
                                      isOk = true;
                                      ownerRightRatio +=
                                        element.propertyRightRatio - 0;
                ...
                if (isOk) {
                  if (ownerRightRatio === 100 || this.contractForm.type === 1) {
                    if (this.contractForm.guestInfo.GuestStoreCode) {
                      ...
                      let guestArr = this.guestList.map(item =>
                        Object.assign({}, item)
                      );
                        ...
                        if (element.name) {
                          if (element.name.replace(/\s/g, "")) {
                            ...
                            if (
                              ...
                              if (
                                element.encryptionMobile.length === 11 ||
                                true
                              ) {
                                let reg = /^1[0-9]{10}$/; //手机号正则
                                let reg_ = /^0\d{2,3}\-?\d{7,8}$/; //固话正则
                      ...
                      if (isOk_) {
                        if (
                          ...
                          if (
                            ...
                            for (
                              let index = 0;
                              index < guestMobileList.length;
                              index++
                            ) {
                              if (
                                ownerMobileList.includes(guestMobileList[index])
                              ) {
                          ...
                          if (
                            ...
                            IdCardList.push(
                              this.contractForm.otherCooperationInfo
                                .identifyCode
                            );
                          }
                          ...
                          if (ownerGuestMobile && otherMobile) {
                            if (IdCardList.length === IdCardList_.length) {
                              if (
                                passportList.length === passportList_.length
                              ) {
                                if (
                                  businessList.length === businessList_.length
                                ) {
                                  if (
                                    ...
                                    if (this.contractForm.isHaveCooperation) {
                                      ...
                                      if (
                                        ...
                                        if (
                                          this.isIdCardNo(
                                            this.contractForm
                                              .otherCooperationInfo.identifyCode
                                          )
                                        ) {
                                          IDcardOk = true;
                                        } else {
              ...
              this.$message({
                message: `房源信息-${
                  this.contractForm.type === 1 ? "租金" : "成交总价"
                }不能为零`,
                type: "warning"
              });
            }
      ...
      if (this.type === 1 && !this.isDeal) {
        ...
        this.$ajax
          ...
          .catch(error => {
            this.fullscreenLoading = false;
            if (
              error !== "该合同房源已被其他合同录入，请重新选择房源！" &&
              error !== "该合同下的房源客源不属于同一个体系，请重新选择！" &&
              error !==
                  ...
                  .then(res => {
                    ...
                    if (res.status === 200) {
                      if (res.data) {
                        ...
                        this.$ajax
                          .get("/api/organize/dep/manager", param3)
                          .then(res => {
                            res = res.data;
                            if (res.status === 200) {
                              if (res.data) {
          ...
          for (var i = 0; i < this.contractForm.contPersons.length; i++) {
            if (this.contractForm.contPersons[i].personType.value === 1) {
              let element = {
                ...
                encryptionMobile: this.contractForm.contPersons[i]
                  .encryptionMobile,
                relation: this.contractForm.contPersons[i].relation,
                propertyRightRatio: this.contractForm.contPersons[i]
                  .propertyRightRatio,
