  ...
  <div class="view-container" ref="tableComView" v-loading="choseLoading">
    ...
    <div class="contract-list">
      ...
      >
        ...
        <el-table-column label="回访录音" min-width="90">
          <template slot-scope="scope">
            <!-- <span v-if="scope.row.isCombine">{{scope.row.signDate.substr(0, 16)}}</span>
            <span v-else>{{Number(scope.row.signDate)|timeFormat_}}</span>-->
...
//       }
    ...
    toDeal(row) {
      this.$ajax.get('/api/contract/copies/getSurplus').then(res=>{
        ...
        if(res.status == 200) {
          let {surplusOnLineQuantity,surplusPaperlessQuantity} = res.data
          if(row.recordType.value == 1 && surplusOnLineQuantity <= 0 ) {
            this.$message.warning('当前权限未开放')
            return
          }else if(row.recordType.value == 10 && surplusPaperlessQuantity <= 0) {
            this.$message.warning('当前权限未开放')
            return
          }else {
            let param = {
    ...
    // 获取合同模板的所有角色签章位置存在情况
    getSignPosition(contCode,isEntrust) {
