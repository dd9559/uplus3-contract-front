    ...
    <div class="data-list">
        ...
        <div class="stepsType gap">
            ...
            <div class="title">
                ...
                <el-button type="primary"
                    @click="addStepsType"
                    v-if="power['sign-set-hq'].state">添加步骤类型</el-button>
            </div>
            <el-table :data="listData"
                ...
                <el-table-column label="操作"
                    ...
                    <template slot-scope="scope">
                        <el-button @click="rowOperation(scope.row,'edit',1)"
                            type="text"
                            size="small"
                            v-if="power['sign-set-hq'].state">编辑</el-button>
                        <el-button @click="rowOperation(scope.row,'delete','stepsType')"
                            type="text"
                            size="small"
                            v-if="power['sign-set-hq'].state">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <div class="tradeSteps gap">
            <div class="title">
                ...
                <el-button type="primary"
                    @click="addTradeSteps"
                    v-if="power['sign-set-hq'].state">添加交易步骤</el-button>
            </div>
            <el-table :data="listData_other"
                ...
                <el-table-column label="步骤附属信息"
                    width="220">
                    <template slot-scope="scope">
                        <p v-if="scope.row.transStepsAttach.length==0">--</p>
                        <p v-for="(item,index) in scope.row.transStepsAttach"
                            :key="index"
                            v-else>{{item.title}}</p>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="是否必须">
                    <template slot-scope="scope">
                        ...
                        <p v-for="(item,index) in scope.row.transStepsAttach"
                            :key="index"
                            v-else>{{item.isRequired==1?'是':'否'}}</p>
                    </template>
                </el-table-column>
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        <el-button type="text"
                            size="medium"
                            @click="rowOperation(scope.row,'edit',2)"
                            v-if="power['sign-set-hq'].state">编辑</el-button>
                        <el-button type="text"
                            size="medium"
                            @click="rowOperation(scope.row,'delete','stepBusiness')"
                            v-if="power['sign-set-hq'].state">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="modalTitle"
            ...
            <el-option v-for="item in roleList" :key="item.key" :label="item.value" :value="item.value"></el-option>
                    ...
                    <el-autocomplete v-model="addForm.dutyType"
                        :fetch-suggestions="querySearch"
                        placeholder="请选择角色"></el-autocomplete>
                </el-form-item>
            </el-form>
            <div slot="footer"
                class="dialog-footer">
                    ...
                    <div class="menu-table">
                        ...
                        <el-table border
                            ...
                            <el-table-column align="center"
                                ...
                                min-width="150">
                                <template slot-scope="scope">
                                    <el-select v-model="tableForm[scope.$index].type"
                                        placeholder="请选择信息类型"
                                        size="small">
                                        <el-option v-for="item in dictionary['561']"
                                            :key="item.key"
                                            :label="item.value"
                                            :value="item.key">
                                        </el-option>
                            ...
                            <el-table-column align="center"
                                ...
                                <template slot-scope="scope">
                                    <el-radio-group v-model="tableForm[scope.$index].isRequired">
                                        <el-radio v-for="item in dictionary['570']"
                                            :key="item.key"
                                            :label="item.value"
                                            :value="item.key"></el-radio>
                                    </el-radio-group>
                                </template>
                            </el-table-column>
                            <el-table-column align="center"
                                label="操作">
                                <template slot-scope="scope">
                                    <el-button @click="rowOperation(scope.row,'delete','info')"
                                        type="text"
                    ...
                    if (res.status === 200) {
                        ...
                        this.listData_other =
                            this.listData.length !== 0
                                ? this.listData[this.rowIndex].stepsList
                                : [];
        ...
        firstCellLight() {
            if (this.allRows.length) {
                for (var i = 1; i < this.allRows.length; i++) {
                    this.allRows[i].firstElementChild.style.background =
                        "#F7FBFF";
            ...
            if (this.addForm.name === "") {
                ...
                if (this.addForm.dutyType) {
                    for (let i = 0; i < this.tempRoleList.length; i++) {
                        if (
                            this.addForm.dutyType === this.tempRoleList[i].value
                        ) {
                ...
                if (flag || !this.addForm.dutyType) {
                    if (this.modalTitle === "添加步骤类型") {
                        this.$ajax
                            .postJSON(
                                `/api/flowmanage/insertStepsType`,
                                this.addForm
                            )
                            .then(res => {
                                res = res.data;
                        ...
                        this.$ajax
                            .postJSON(`/api/flowmanage/updateStepsType`, param)
                            .then(res => {
                                res = res.data;
                                if (res.status === 200) {
                                    this.$message("修改成功");
            ...
            if (opera === "edit") {
                if (type === 1) {
                    ...
                    this.tableForm = JSON.parse(
                        JSON.stringify(row.transStepsAttach)
                    );
                    this.tableForm.forEach(item => {
                ...
                if (type === "info") {
                    ...
                    this.$confirm("是否删除该信息?", "删除步骤类型", {
                        ...
                        this.rowIndex = 0;
                        this.$ajax
                            .post("/api/flowmanage/deleteStepsType", param)
                            .then(res => {
                                res = res.data;
                                if (res.status === 200) {
                                    this.$message({
