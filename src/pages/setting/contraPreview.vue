  ...
  <div class="view-container" v-loading="loading">
    ...
    <div class="top">
      <div class="type" v-if="uploadType == 3">
        <div :class="{ active: isActive === 1 }" @click="changeType(1)">
          全款买卖合同
        </div>
        <div :class="{ active: isActive === 2 }" @click="changeType(2)">
          贷款买卖合同
        </div>
        <div :class="{ active: isActive === 3 }" @click="changeType(3)">
          居间合同
        </div>
      </div>
    ...
    <div class="bodycontainer" ref="bigbox">
      <div class="ht-list listone" v-show="showFir" ref="htlist">
        <div class="pagerUp">
          ...
          <el-button
            class="el-icon-caret-bottom paper-next"
            @click="add(1)"
          ></el-button>
        </div>
        <img id="ht" src="" alt="" />
...
export default {
  ...
  mounted() {
    if (this.show == 1) {
      ...
      this.$ajax
        .get("/api/setting/contractTemplate/show", {
          enableTemplateId: this.enableTemplateId,
        })
        .then((res) => {
          this.$nextTick(() => {
            this.loading = false;
          ...
          if (resadd.img && resadd.img.url != "") {
            ...
            var newsrc =
              this.imgSrc.substr(0, this.imgSrc.lastIndexOf(".")) +
              this.count +
              this.imgSrc.substr(this.imgSrc.lastIndexOf("."));
            this.autograph(htImg, newsrc);
          } else if (
            res.data.data.businessImg &&
            res.data.data.businessImg.url != ""
          ) {
            this.uploadType = 2;
            ...
            res.data.data.wholeImg &&
            res.data.data.wholeImg.url != ""
          ) {
            this.uploadType = 3;
  ...
  methods: {
    changeType(type) {
      if (type == 2) {
        ...
        var newsrc2 =
          this.imgSrc2.substr(0, this.imgSrc2.lastIndexOf(".")) +
          this.count2 +
          this.imgSrc2.substr(this.imgSrc2.lastIndexOf("."));
        ...
        var newsrc =
          this.imgSrc3.substr(0, this.imgSrc3.lastIndexOf(".")) +
          this.count3 +
          this.imgSrc3.substr(this.imgSrc3.lastIndexOf("."));
        this.autograph(htImg, newsrc);
      }
    },
    saveAll() {
      if (this.show == 1) {
          ...
          param = Object.assign({}, param, {
            address: {
              whole:
                this.qkAddress == ""
                  ? ""
                  : this.qkAddress.path + "?" + this.qkAddress.name,
              loan:
                this.dkAddress == ""
                  ? ""
                  : this.dkAddress.path + "?" + this.dkAddress.name,
              residence:
                this.jjianAddress2 == ""
                  ? ""
                  : this.jjianAddress2.path + "?" + this.jjianAddress2.name,
            },
            imgAddress: {
              whole: this.imgSrc,
              loan: this.imgSrc2,
          ...
          param = Object.assign({}, param, {
            address: {
              address:
                this.mbanAddress == ""
                  ? ""
                  : this.mbanAddress.path + "?" + this.mbanAddress.name,
              business:
                this.mmaiAddress == ""
                  ? ""
                  : this.mmaiAddress.path + "?" + this.mmaiAddress.name,
              residence:
                this.jjianAddress == ""
                  ? ""
                  : this.jjianAddress.path + "?" + this.jjianAddress.name,
            },
            imgAddress: {
        ...
        this.$ajax
          .postJSON("/api/setting/contractTemplate/insert", param)
          .then((res) => {
            if (res.status == 200) {
              this.touch = false;
              this.$message({
    ...
    del(type) {
      if (type == 1) {
        ...
        var newsrc =
          this.imgSrc2.substr(0, this.imgSrc3.lastIndexOf(".")) +
          this.count3 +
          this.imgSrc3.substr(this.imgSrc3.lastIndexOf("."));
    ...
    autograph(obj, newsrc) {
      ...
      for (let i = 0; i < this.qmnewsrcArr.length; i++) {
        if (this.qmnewsrcArr[i][newsrc]) {
          ...
          flag = 1;
          if (
            Number(this.qmnewsrcArr[i].timeout) <
      ...
      if (flag == 0) {
        this.$ajax
          .get("/api/load/generateAccessURL", { url: newsrc })
          .then((res) => {
            if (res.status == 200) {
              this.timeout = res.data.data.url
                .split("?")[1]
                .split("&")[0]
                .split("=")[1];
    ...
    getImgAdd(count) {
      ...
      this.$ajax
        .get("/api/setting/contractTemplate/checkTemplate", param)
        .then((res) => {
          if (res.status == 200) {
            this.$nextTick(() => {
              this.loading = false;
