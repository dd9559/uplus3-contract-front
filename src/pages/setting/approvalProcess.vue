      ...
      <div class="content">
        ...
        <div class="input-search">
          ...
          <el-select
            ...
            @change="changeFlowTypeOne"
            :clearable="true"
            @clear="clearCondition"
            filterable
          >
            <el-option
              v-for="item in dictionary['573']"
              v-if="item.key!=4"
              :key="item.key"
              :label="item.value"
              :value="item.key"
          ...
          <el-select
            size="small"
            v-model="searchForm.branchCondition"
            :clearable="true"
            class="w200 ml-10"
          >
            <el-option
              v-for="item in homeConditionList"
              :key="item.key"
              :label="item.value"
              :value="item.key"
            ></el-option>
          ...
          <el-table-column label="交易类型" prop="modularType">
            <template slot-scope="scope">
              <span
                v-for="item in dictionary['711']"
                :key="item.key"
                v-if="item.key===scope.row.modularType"
              >{{item.value}}</span>
            </template>
          </el-table-column>
          <el-table-column label="体系" prop="systemTag" :formatter="nullFormatter">
            <template slot-scope="scope">
              <span
                ...
                :key="item.key"
                v-if="item.key===scope.row.systemTag"
              >{{item.value}}</span>
            </template>
          </el-table-column>
          <el-table-column label="部门" prop="deptName" :formatter="nullFormatter"></el-table-column>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button
                ...
                v-if="power['sign-set-verify'].state"
              >编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div class="aduit-content">
        <div class="row">
          ...
          <div class="aduit-input must w50">
            ...
            <el-select
              size="small"
              v-model="aduitForm.type"
              @change="changeFlowTypeTwo"
              :disabled="editDisabled"
              @clear="clearCondition(2)"
              :clearable="true"
              filterable
            >
              <el-option
                v-for="item in dictionary['573']"
                v-if="aduitForm.modularType==0?item.key!=4:item.key==1"
                :key="item.key"
                :label="item.value"
                :value="item.key"
              ></el-option>
            ...
            <li v-for="(item,index) in nodeList" :key="index">
              <div class="node-body">
                ...
                <el-select
                  ...
                  @change="checkSystemFn(index)"
                  clearable
                  @clear="clearFn(index)"
                >
                  <el-option
                    v-for="m in aduitTypeArr"
                    :key="m.key"
                    :label="m.value"
                    :value="m.key"
                    :disabled="forbidFn(item.type,m.key)"
                  ></el-option>
                </el-select>
                <!-- 人员 -->
                <div v-if="item.type===0" class="person">
                  <select-tree
                    ...
                    @search="searchDep($event,index)"
                  ></select-tree>
                  <el-select
                    class="person-right"
                    :clearable="true"
                    v-loadmore="moreEmploye1"
                    size="small"
                    v-model="item.personArr"
                    placeholder="请选择"
                    filterable
                    multiple
                    @change="multiSelect(item.type,index)"
                    @focus="getNodeIndex(index)"
                  >
                    <el-option
                ...
                <div v-if="item.type===4&&version==3">
                  <div class="person">
                    <el-select
                      size="small"
                      placeholder="请选择部门类型"
                      v-model="item.depType"
                      filterable
                      @change="getDepStr($event,index)"
                    >
                      <el-option
                    ...
                    <el-select
                      ...
                      placeholder="请选择职级类型"
                      v-if="item.depType"
                      v-model="item.depTypeArr"
                      filterable
                      multiple
                      @change="multiSelect(item.type,index)"
                    >
                      <el-option
                        v-for="item in dictionary['659']"
                        :key="item.key"
                        :label="item.value"
                        :value="item.key"
                      ></el-option>
                ...
                <div class="row-icon">
                  <span class="button" @click="addRow">
                    <i class="icon el-icon-plus"></i>
                  </span>
                  <span class="button" @click="removeRow(index)">
              ...
              <div class="default" v-show="item.choice&&item.choice.length>0&&index!==0">
                ...
                <div class="multiple" ref="curChoice">
                  ...
                  >
                    ...
                    <i
                      class="el-icon-close"
                      @click.stop="delChoice(index,item.choice,m)"
                    ></i>
                  </span>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    getData(type = "init") {
      ...
      this.$ajax
        .postJSON("/api/auditflow/selectFlowList", param)
        .then((res) => {
          res = res.data;
          if (res.status === 200) {
            this.tableData = res.data.data;
    ...
    getAduitType() {
      this.$ajax
        .get("/api/auditflow/getApprovalType")
        .then((res) => {
          res = res.data;
          if (res.status === 200) {
    ...
    getBranchObj() {
      this.$ajax
        .get("/api/auditflow/getAuditBranch")
        .then((res) => {
          res = res.data;
          if (res.status === 200) {
    ...
    getDeps(key) {
      this.$ajax
        .get("/api/organize/systemtag/deps", { systemTag: key })
        .then((res) => {
          res = res.data;
          if (res.status === 200) {
            this.depList = res.data;
    ...
    moreEmploye1() {
      if (
        this.nodeList[this.nodeIndex].employeList.length >=
        this.nodeList[this.nodeIndex].employeeTotal
      ) {
        return;
      } else {
        this.depHandleClick(
          ...
          this.nodeIndex,
          ++this.nodeList[this.nodeIndex].employeePage
        );
      }
    },
    ...
    depHandleClick(data, index, page = 1) {
      ...
      this.$ajax
        ...
        .then((res) => {
          res = res.data;
          if (res.status === 200) {
            this.nodeList[index].employeList = this.nodeList[
              index
            ].employeList.concat(res.data.list);
    ...
    operation(row) {
      ...
      for (let i = 1; i < array.length; i++) {
        ...
        Object.assign(array[i], {
          ...
          depType: array[i].choice.filter((e) => e.type === 4).length
            ? array[i].choice.filter((e) => e.type === 4)[
                array[i].choice.filter((e) => e.type === 4).length - 1
              ].userId
            : "",
          depTypeStr: array[i].choice.filter((e) => e.type === 4).length
            ? array[i].choice.filter((e) => e.type === 4)[
                array[i].choice.filter((e) => e.type === 4).length - 1
              ].userName
            : "",
      ...
      this.$ajax
        .get("/api/organize/systemtag/deps", {
      ...
      if(this.isCrossSystem) delete para2.systemTag
      this.$ajax
        .get("/api/access/systemtag/deps", para2)
        .then((res) => {
          res = res.data;
          if (res.status == 200) {
            this.depsList = res.data;
    ...
    setConditionList(val, type = 1) {
      for (let key in this.branchObj) {
        if (val == key) {
          this[
            type == 1 ? "homeConditionList" : "conditionList"
          ] = this.branchObj[key];
          break;
        }
      }
    ...
    getDepStr(e, i) {
      ...
      let arr = this.nodeList[i].choice.filter(
        (item) => item.userId === this.nodeList[i].depType
      );
    ...
    multiDel(index, type, ar, t) {
      let arr = this.nodeList[index].choice.filter((item) => {
        return type !== 4
          ? item.type === type
          : item.type === type && item.userId === this.nodeList[index].depType;
      });
      let arr1 = [];
      arr.forEach((item) => {
      ...
      this.nodeList[index].choice.forEach((item, i) => {
        if (
          type !== 4
            ? item.userId === arr2[0]
            : item.positionId === arr2[0] &&
              item.userId === this.nodeList[index].depType
        ) {
    ...
    multiSelect(type, index) {
      if (type === 0) {
        ...
        if (
          ...
          this.nodeList[index].personArr.length
        ) {
          for (var i = 0; i < this.nodeList[index].employeList.length; i++) {
            if (
              this.nodeList[index].personArr[
                this.nodeList[index].peopleTime - 1
              ] === this.nodeList[index].employeList[i].empId
            ) {
        ...
        // 部门名称
        if (
          this.nodeList[index].depsTime === this.nodeList[index].depArr.length
        ) {
          this.$ajax
            .get("/api/organize/selectEmpByDepType", {
              depId: this.nodeList[index].depArr[
                this.nodeList[index].depsTime - 1
              ],
            })
            .then((res) => {
              res = res.data;
              if (res.status === 200) {
                if (res.data > 0) {
                  ...
                  this.$message({
                    message: "该部门下没有人员，请选择其他部门",
                  });
                  this.nodeList[index].depArr.splice(
                    this.nodeList[index].depArr.length - 1,
                    1
                  );
                }
              }
        ...
        // 角色
        if (
          this.nodeList[index].rolesTime === this.nodeList[index].roleArr.length
        ) {
          for (var i = 0; i < this.roleList.length; i++) {
            if (
              this.nodeList[index].roleArr[
                this.nodeList[index].rolesTime - 1
              ] === this.roleList[i].key
            ) {
              this.nodeList[index].choice.push({
                type: 2,
        ...
        if (
          this.nodeList[index].depTypeTime ===
          this.nodeList[index].depTypeArr.length
        ) {
        ...
        // 职务名称
        if (
          this.nodeList[index].jobTime === this.nodeList[index].jobArr.length
        ) {
          for (var i = 0; i < this.jobNameList.length; i++) {
            if (
              this.nodeList[index].jobArr[this.nodeList[index].jobTime - 1] ===
              this.jobNameList[i].Value
            ) {
              this.nodeList[index].choice.push({
                type: 5,
      ...
      let ar = this.nodeList[index].choice.filter(
    ...
    checkEmp(url, type, index, ar, t, id_, s) {
      ...
      for (var i = 0; i < this.dictionary["659"].length; i++) {
        if (
          this.nodeList[index][ar][this.nodeList[index][t] - 1] ===
          this.dictionary["659"][i].key
        ) {
      ...
      this.$ajax.get(url, param).then((res) => {
        ...
        if (res.status === 200) {
          if (res.data) {
            ...
            this.nodeList[index][ar].splice(
              this.nodeList[index][ar].length - 1,
              1
            );
          }
        }
    ...
    getJobName(cityId, systemTag) {
      this.$ajax
        .get("/api/employee/getPositionRank", { cityId, systemTag })
        .then((res) => {
          res = res.data;
          if (res.status == 200) {
    ...
    aduitPost(param, msg) {
      console.log(param);
      this.$ajax
        .postJSON("/api/auditflow/operateFlow", param)
        .then((res) => {
          res = res.data;
          if (res.status === 200) {
