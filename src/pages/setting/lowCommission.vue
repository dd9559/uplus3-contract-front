  ...
  <div class="view-container" ref="tableComView">
    ...
    <div class="contract-list">
      <div class="listTitle">
        ...
        <div>
          <el-button class="btn-info" round type="primary" size="small" @click="dialogRatio=true">新增低佣比例</el-button>
        </div>
      </div>
    ...
    save(){
      if(this.systemType){
        ...
        for(var i=0;i<this.ratioList.length;i++){
          isOK=false
          if((this.ratioList[i].dealPriceMin.toString()).length>0&&Number(this.ratioList[i].dealPriceMax)>0){
            if((this.ratioList[i].comPercentMin.toString()).length>0){
              if(Number(this.ratioList[i].dealPriceMax)>Number(this.ratioList[i].dealPriceMin)){
                isOK=true
                // if(Number(this.ratioList[i].comPercentMax)>Number(this.ratioList[i].comPercentMin)){
                //   isOK=true
                // }else{
                //   this.$message({
                //     message:"低佣比例范围错误",
        ...
        if(isOK){
          ...
          let listLength=this.ratioList.length
          if(listLength>1){
            let beginList = []
            let endList = []
            this.ratioList.forEach(element => {
