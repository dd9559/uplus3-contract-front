<template>
  ...
  >
    <div>
      <div class="paper-edit-box" v-if="paperType">
        <ul>
          <li>
            <p>
              <label>合同编号:</label>
              <span>{{paperInfoData.contCode}}</span>
            </p>
            <p>
              <label>物业地址:</label>
          ...
          <li class="tips-second">
            <p>
              <label>合计金额:</label>
              <span>{{paperInfoData.proceedsAmount}}元</span>
            </p>
            <p>
              <label>交款单位:</label>
              <el-select
        ...
        <el-button
          round
          size="small"
          class="paper-btn paper-btn-blue paper-btn-float"
          @click="billing"
        >确定开票</el-button>
      </div>
        ...
        <LayerPrint ref="easyPrint" :morePaper="opera2batch">
          <template v-if="!opera2batch">
            <LayerPaperInfo
              ...
              :storeReceiveList="paperInfoData.storeDivide?paperInfoData.storeDivide:[]"
            ></LayerPaperInfo>
          </template>
          <template v-else>
            <LayerPaperInfo
              ...
              :storeReceiveList="item.storeDivide?item.storeDivide:[]"
            ></LayerPaperInfo>
          </template>
        </LayerPrint>
      ...
      <p
        class="print-waring"
        v-if="FooterShow&&printType!=='all'&&getUser.version!==3"
      >温馨提示：打印时，请选择谷歌浏览器</p>
    </div>
...
export default {
  ...
  methods: {
    ...
    getImgFn(url, batch = false) {
      if (batch) {
        this.$ajax
          .put("/api/load/generateAccessURLBatch", { urls: url }, 2)
          .then(res => {
            res = res.data;
            if (res.status === 200) {
      ...
      if (!!url) {
        url = url.split("?")[0];
        this.$ajax
          .get("/api/load/generateAccessURL", {
            url
          })
          .then(res => {
            res = res.data;
    ...
    // 票据详情 打印
    printPaper() {
      if (this.paperType && !this.form.payerName) {
        //开票时判断是否选择单位
        this.$message.error("请选择或输入交款单位");
        return false;
      }
      ...
      if (this.getPrint === "client" && this.paperType && this.city_wh) {
        //武汉开票添加signatureType字段
        Object.assign(obj, { signatureType: this.form.signatureType });
      }
      this.$ajax
        .post(
          this.getPrint === "client"
            ? "/api/bills/print"
            : this.opera2batch
            ? "/api/bills/batchPrintTally"
            : "/api/bills/printTally",
          obj
        )
        .then(res => {
          res = res.data;
  ...
  computed: {
    ...
    getPro: function() {
      ...
      let tips = types.findIndex(item => {
        if (
          this.moneyTypes.length > 0 &&
          item.list.includes(this.moneyTypes[this.activeType].project)
        ) {
          return item;
        }
      });
